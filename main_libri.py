        ...
        for file in files:
            if file.endswith('.wav'):
                with open(os.path.join(root, file), 'rb') as f:
                    ...
                    inputs = []
                    with open(os.path.join(root, file.split('.')[0]) + '.lab') as g:
                        phones = g.readlines()
                        for phone in phones:
...
def write_speaker_data(spk):
    speaker_data = []
    files = [os.path.join(spk, file) for file in os.listdir(spk) if file.endswith('.npy')]
    for file in files:
        speaker_data.append(np.load(file))
    speaker_data = np.concatenate(speaker_data, axis=0)
    speaker_data = speaker_data[:len(speaker_data) - len(speaker_data) % NUM_FRAMES]
    speaker_data = np.reshape(speaker_data, (-1, INPUT_SIZE))
    np.save(os.path.join(SPEAKER_DATA_DIR, os.path.basename(spk)), speaker_data)


def create_speaker_files():
    train_dir = os.path.join(DATA_DIR, 'train-clean100')
    train_spk = os.listdir(train_dir)
    test_dir = os.path.join(DATA_DIR, 'test-clean')
    test_spk = os.listdir(test_dir)
    spk_list = [os.path.join(train_dir, spk) for spk in train_spk]
    spk_list.extend([os.path.join(test_dir, spk) for spk in test_spk])

    pool = Pool()
    pool.map(write_speaker_data, iter(spk_list))
...
def create_train_valid_data():
    ...
    train_spk = os.listdir(os.path.join(DATA_DIR, 'train-clean100'))

    files = [os.path.join(SPEAKER_DATA_DIR, spk + '.npy') for spk in train_spk]
...
def train_vector_model():
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.33, patience=3, min_lr=0.0000001)
...
def match_couples():
    ...
    for spk in train_spk:
        spk_path = os.path.join(DATA_DIR, 'train-clean100', spk)
        train_files.extend(([(spk, file) for file in os.listdir(spk_path) if file.endswith('.npy')]))

    with open(os.path.join(DATA_DIR, 'train_matches.txt'), 'w') as g:
        for i in range(len(train_files) - 1):
            j = i + 1
            if train_files[i][0] == train_files[j][0]:
    ...
    test_spk = os.listdir(os.path.join(DATA_DIR, 'test-clean'))

    test_files = []
    for spk in test_spk:
        spk_path = os.path.join(DATA_DIR, 'test-clean', spk)
        test_files.extend(([(spk, file) for file in os.listdir(spk_path) if file.endswith('.npy')]))
...
def extract_dvector(model, feats):
    feats = feats[:len(feats) - len(feats) % NUM_FRAMES]
    feats = np.reshape(feats, (-1, INPUT_SIZE))
...
def create_metric_train_data(vector_model):
    with open(os.path.join(DATA_DIR, 'train_matches.txt')) as f:
        matches = f.readlines()
    count = 0
    x_train = np.empty((3 * len(matches), 512))
    y_train = np.empty((3 * len(matches), 1))
    for i in range(len(matches)):
        tokens = matches[i].split()
        anch = extract_dvector(vector_model, np.load(os.path.join(DATA_DIR, 'train-clean100\\{}\\{}'.format(tokens[0].split('-')[0], tokens[0]))))
        same = extract_dvector(vector_model, np.load(os.path.join(DATA_DIR, 'train-clean100\\{}\\{}'.format(tokens[1].split('-')[0], tokens[1]))))
        diff = extract_dvector(vector_model, np.load(os.path.join(DATA_DIR, 'train-clean100\\{}\\{}'.format(tokens[2].split('-')[0], tokens[2]))))
        diff2 = extract_dvector(vector_model, np.load(os.path.join(DATA_DIR, 'train-clean100\\{}\\{}'.format(tokens[3].split('-')[0], tokens[3]))))
        if len(anch) == 0 or len(same) == 0 or len(diff) == 0 or len(diff2) == 0:
            continue
        x_train[count] = np.concatenate((anch, same))
        y_train[count] = 1
        x_train[count + 1] = np.concatenate((anch, diff))
        y_train[count + 1] = 0
        x_train[count + 2] = np.concatenate((anch, diff2))
        y_train[count + 2] = 0
        count += 3
...
def create_metric_train_data_sre04(vector_model):
    with open(os.path.join(DATA_DIR, 'train_matches.txt')) as f:
        matches = f.readlines()
    count = 0
...
def create_metric_test_data(vector_model):
    ...
    for i in range(len(matches)):
        tokens = matches[i].split()
        anch = extract_dvector(vector_model, np.load(os.path.join(DATA_DIR, 'test-clean\\{}\\{}'.format(tokens[0].split('-')[0], tokens[0]))))
        same = extract_dvector(vector_model, np.load(os.path.join(DATA_DIR, 'test-clean\\{}\\{}'.format(tokens[1].split('-')[0], tokens[1]))))
        diff = extract_dvector(vector_model, np.load(os.path.join(DATA_DIR, 'test-clean\\{}\\{}'.format(tokens[2].split('-')[0], tokens[2]))))
        if len(anch) == 0 or len(same) == 0 or len(diff) == 0:
            continue
